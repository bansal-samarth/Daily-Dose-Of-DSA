...
public:
    int idealArrays(int n, int maxValue) {
        ...
        // dp[i][j] := the number of strictly increasing ideal arrays of length
        // i ending in j dp[i][j] := sum(dp[i - 1][k]), where j % k == 0
        ...
        for (int i = 1; i <= maxLength; ++i) {
            // nCk(n - 1, i - 1) := the number of ways to create an ideal array
            // of length n from a strictly increasing array of length i
