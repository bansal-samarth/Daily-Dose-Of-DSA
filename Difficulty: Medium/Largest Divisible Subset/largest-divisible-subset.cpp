class Solution {
  public:
    ...
    // void solve(vector<int>& arr, vector<int>& output, vector<int>& ans, int prev, int idx) {
    //     if(idx == arr.size()) {
    ...
    //     // SKIP
    //     solve(arr, output, ans, prev, idx + 1);
        
    //     // TAKE
    //     if(prev == -1 || (arr[idx] % arr[prev] == 0)) {
    //         output.push_back(arr[idx]);
    //         solve(arr, output, ans, idx, idx + 1);
    //         output.pop_back();
    //     }
    // }
